...
def transaction_4():
    """
    Rename depot d1 → d10 in BOTH Depot and Stock.
    Must update Stock first, then Depot.
    """
    conn = connect_db()
    if conn is None: return
    cur = conn.cursor()
    ...
    try:
        print("\nRunning Transaction 4: Rename depot d1 to d10...")

        # update Stock first
        cur.execute("UPDATE Stock SET depotID = 'd10' WHERE depotID = 'd1';")

        # update Depot
        cur.execute("UPDATE Depot SET depotID = 'd10' WHERE depotID = 'd1';")
